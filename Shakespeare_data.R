...
for (my.author in my.authors) {
  ...
  df <- df.preprocessed %>%
    ...
    dplyr::select(cerl_control_number = original_row, title, publication_year, language_primary, 
                  publication_place, publication_country, publisher) %>%
    dplyr::arrange(cerl_control_number, title, publication_year, language_primary,
                   publication_place, publication_country, publisher)
