...
We have `r sum(tab)` documents with country and original gatherings information.
...
```{r docsizes-3, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=8}
...
for (sel.gatherings in top.gatherings) {
  ...
  top.countries <- names(which(sort(sapply(split(mydf$n, mydf$country), sum)) > 20))
